...
[DIANA/HEP Meeting](https://indico.cern.ch/event/759480/)
...
- No ROOT dependency required at all! (Thanks [uproot](https://github.com/scikit-hep/uproot)!)
...
.grid[
.kol-2-3[
With ML backends in `pyhf` gain access to exact (higher order) derivatives of the likelihood during minimization &mdash; accuracy is only limited by floating point precision
...
- Currently benchmarking CPU vs. GPU performance with [SMU's M2 HPC cluster](http://faculty.smu.edu/csc/documentation/about.html)
- First GPU and TPU results running on Google Colab show order of magnitude speedup for interpolators on large models

.center.width-80[![GPU](figures/GPU_TPU_times.png)]
.center[Single channel with $n$ bins (uncorrelated nuisance parameters).]
...
- Needs to be revisited with recent updates that properly .bold[implement vectorization] and graph structure
   - Seeing over a $100x$ speedup to that seen in image for single channel
...
.center.width-70[[![Binder](https://mybinder.org/badge.svg)](https://mybinder.org/v2/gh/diana-hep/pyhf/talk/DIANA-HEP-talk?filepath=docs%2Fexamples%2Fnotebooks%2Ftalks%2FDIANA-HEP-Demo.ipynb)]
...
>>> import pyhf.utils
>>>
>>> pdf = pyhf.simplemodels.hepdata_like(signal_data=[12.0, 11.0],
                                         bkg_data=[50.0, 52.0],
                                         bkg_uncerts=[3.0, 7.0])
...
.center.width-100[[![GitHub_awesome](figures/GitHub_banner.png)](https://github.com/diana-hep/pyhf)]
...
- There are already projects like [GooFit](https://github.com/GooFit/GooFit), which nicely handle unbinned models.
   - "... massively parallel fitter for OpenMP and CUDA for C++ and Python"
